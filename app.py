...
# ✨ [기능 추가] 지역별 최적 가중치 엔진 (이미지 2, 3 내용 반영) ✨
def get_regional_weight(address):
    address = str(address)
    if '송도' in address: return 1.15   # 송도 지역 가중치
    if '동춘' in address: return 1.08   # 동춘 지역 가중치
    if '원인재' in address: return 1.05 # 원인재 지역 가중치
    return 1.0

filtered_df['지역가중치'] = filtered_df['주소'].apply(get_regional_weight)
...
# 최종 점수 계산 (기본점수 * 지역가중치 - 예산패널티)
filtered_df['최종점수'] = ((filtered_df['기본점수'] * filtered_df['지역가중치']) - filtered_df['예산패널티']).round(1)
